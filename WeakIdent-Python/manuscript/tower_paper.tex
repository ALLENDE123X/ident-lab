...
\caption{Cumulative distribution of regret. The near-vertical rise at $x=0$ indicates that 97.06\% of held-out test predictions achieve zero regret.}
...
Figure~\ref{fig:features} shows the Random Forest feature importances. The $\sigma(u_{xx})$ (feature 1) is most predictive at 23.2\%, followed by $\sigma(u_{xxx})$ (feature 2) at 16.2\%, and $\sigma(u_x)$ (feature 0) at 11.5\%. These spatial derivative statistics capture the complexity of the dynamics, which strongly influences method performance.
...
\begin{itemize}
    ...
    \item Under 2\% Gaussian noise, RobustIDENT becomes optimal for 20.5\% of windows, while LASSO drops to 44.5\% and STLSQ to 35\%.
    \item Under 5\% Gaussian noise, RobustIDENT captures 19\% of windows.
    \item Under outlier corruption (1\% spikes), RobustIDENT is optimal for 19\% of windows.
\end{itemize}

These results confirm that robust methods become competitive under adverse conditions, motivating the need for adaptive method selection rather than a one-size-fits-all approach.
...
We presented a meta-learning framework for automated PDE identification method selection. By extracting 12 inexpensive features from raw data, a Random Forest classifier predicts with 97.06\% test accuracy which of four identification methods will perform best. On our held-out test set of 1,158 windows, 97.06\% of predictions matched the oracle's choice, achieving zero regret. This framework enables efficient algorithm selection for PDE discovery, avoiding the computational cost of running all methods on every data window. Future work will extend the approach to noisy data regimes, implement uncertainty-based safety mechanisms, and evaluate cross-PDE generalization.

\section*{Acknowledgements}

The author thanks Dr.\ Sung Ha Kang (Georgia Tech School of Mathematics) for guidance and for developing the WeakIDENT framework that this work extends. The WeakIDENT implementation is based on the original codebase by Mengyi Tang and collaborators \citep{tang2023weakident}. The author also thanks the Georgia Tech School of Mathematics for computational resources. This work was supported in part by undergraduate research opportunities at Georgia Tech.
