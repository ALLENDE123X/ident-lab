...
\begin{itemize}
    ...
    \item \textbf{Tree ensembles} (Random Forest \citep{breiman2001random}, Gradient Boosting \citep{friedman2001greedy}): Handle nonlinearities and feature interactions naturally, are robust to feature scaling, and typically achieve strong performance on tabular, heterogeneous feature sets like the Tiny-12 features.
\end{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{noise_best_method_distribution.png}
...
\begin{itemize}
    \item On clean data, LASSO wins 56.5\% and STLSQ wins 43.5\% of windows (consistent with the main results).
...
\textbf{Skewed class distribution:} On clean synthetic data, LASSO and STLSQ dominate, accounting for 99.9\% of best-method selections. WeakIDENT and RobustIDENT collectively appear as optimal in only 0.1\% of windows. This reflects the favorable conditions of our synthetic experiments (low noise, sufficient resolution). Under higher noise levels, outlier-corrupted data, or coarser spatial grids, weak-form methods like WeakIDENT are expected to outperform sparse regression approaches due to their inherent noise robustness through integration. Future work should systematically evaluate method performance across a noise-resolution parameter sweep to demonstrate regimes where WeakIDENT and RobustIDENT become the preferred methods.
