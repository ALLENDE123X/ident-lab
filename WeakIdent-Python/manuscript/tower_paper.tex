...
The training pipeline consists of: (1) loading the labeled dataset with features and best-method labels; (2) standardizing features using z-score normalization; (3) splitting into 80\% training and 20\% test sets with stratification; and (4) training each classifier and evaluating on the held-out test set. Stratification is best-effort; for extremely rare classes (e.g., RobustIDENT with 2 total samples), the 80/20 split may yield zero test examples.
...
\textbf{Preprocessing leakage:} Feature standardization was computed using global dataset statistics prior to the train/test split. This introduces minor data leakage and may make the reported test performance slightly optimistic; future work will enforce train-only preprocessing.
