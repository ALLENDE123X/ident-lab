...
Random Forest achieved the highest test accuracy at 97.06\%, followed by Gradient Boosting at 95.68\%. Cross-validation used default 5-fold KFold (non-stratified) on the full dataset. The CV mean accuracy is lower than test accuracy (87.9\% vs 97.1\%) with high variance (std $\approx$ 0.12), likely because non-stratified folds can have unrepresentative class distributions when minority classes are extremely rare (only 6 samples in WeakIDENT/RobustIDENT combined). The stratified test split provides a more stable and representative evaluation.
